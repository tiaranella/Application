    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10">
          <div>
            <h2 className="text-xl font-display font-bold text-gray-900">Create New Event</h2>
            <p className="text-xs text-gray-500 font-medium mt-0.5">Fill in the details to host your next gathering</p>
          </div>
        ...
        <form onSubmit={handleCreateEvent} className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <div className="space-y-1.5">
            ...
            <input type="text" value={eventTitle} onChange={(e) => setEventTitle(e.target.value)} placeholder="e.g., Tech Conference 2025" className="w-full px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-[#1e3a8a] focus:border-[#1e3a8a] transition-all text-sm" />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="What makes your event special?" rows={3} className="w-full px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-[#1e3a8a] focus:border-[#1e3a8a] transition-all text-sm resize-none" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g., Convention Center" className="w-full px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-[#1e3a8a] text-sm" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-700 uppercase tracking-wide">Capacity</label>
              <input type="number" value={capacity} onChange={(e) => setCapacity(e.target.value)} placeholder="Empty = Unlimited" className="w-full px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-[#1e3a8a] text-sm" />
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3 rounded-b-2xl">
          ...
          <button type="submit" onClick={handleCreateEvent} disabled={isSubmitting} className="px-6 py-2.5 text-sm font-bold rounded-xl text-white bg-[#1e3a8a] hover:bg-blue-800 transition-colors flex items-center gap-2 disabled:opacity-70">
            {isSubmitting ? <span className="loading loading-spinner loading-xs"></span> : "Create Event"}
          </button>
        </div>
      </div>
    </div>
